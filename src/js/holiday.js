...
window.addEventListener('load', async () => {
  var data = await db.collection('holiday_courses').get();
  ...
  for (var course of courses) {
    ...
    if (!course.disabled) {
      ...
      TIME.innerHTML = `
            ...
            <div class="time"><strong>Date:</strong> ${generateEnglishDate(
              course.date_from,
              course.date_to
            )}</div>
...
function parsePrice(price) {
  ...
  return `R ${Number(price).toLocaleString()}.00/person per day`;
}

function generateEnglishDate(date_from, date_to) {
  if (date_from === date_to) {
    return parseDate(date_from);
  } else return `${parseDate(date_from)} - ${parseDate(date_to)}`;
}
